...
$(function () {
     ...
     * 获取登陆用户名
     */
    $("#myName").val(sessionStorage.getItem("user_name"));
    $("#MateName").val(sessionStorage.getItem("mate_name"));
    ...
    $("#clearBtn").click(function () {
        $("#historyMsg").empty();//移除被选元素的所有子节点和内容
        $("#messageInput").focus();
    });
...
function initEmoji() {
    ...
    for (var i = 71; i > 0; i--){
        ...
        emojiItem.src = $("#emojiBaseUrl").val().trim() + i + ".gif";
        emojiItem.title = i;
...
function connect() {
    ...
    stompClient.connect({}, function(frame){
        stompClient.subscribe("/topic/chat/message", function (message) {
            ...
            if(user===username||user==matename) {
                if (messageType == "text"){
...
function showEmoji(message) {
    ...
    while (match = regrex.exec(message)){
        ...
        var emojiUrl = $("#emojiBaseUrl").val().trim() + emojiIndex + ".gif";
